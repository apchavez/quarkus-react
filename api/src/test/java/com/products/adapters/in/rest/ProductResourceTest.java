...
class ProductResourceTest extends BaseMongoIntegrationTest {
    ...
    @Test
    void findAllActive_returnsPagedResponse() {
        createAndGetId();
    ...
    @Test
    void findAllActive_sizeExceedsMax_returns400() {
        given()
    ...
    @Test
    void findAllActive_negativePageNumber_returns400() {
        given()
    ...
    @Test
    void findAllActive_noToken_returns401() {
        given()
            .queryParam("page", 0).queryParam("size", 10)
        .when()
            .get(BASE + "/active")
    ...
    @Test
    void findAllInactive_returnsOnlyInactiveProducts() {
        String id = createAndGetId();
        String deactivateBody = """
                {"sku":"SKU-RT-001","name":"Resource Test Product","description":"Test description","category":"Technology","price":99.99,"stock":5,"active":false}
                """;
        given()
            .contentType(ContentType.JSON).body(deactivateBody)
            .header("Authorization", "Bearer " + TestTokenUtil.adminToken())
        .when()
            .put(BASE + "/{id}", id);
    ...
    @Test
    void findAllInactive_sizeExceedsMax_returns400() {
        given()
            .queryParam("page", 0).queryParam("size", 101)
            .header("Authorization", "Bearer " + TestTokenUtil.userToken())
        .when()
    ...
    @Test
    void findAllInactive_noToken_returns401() {
        given()
            .queryParam("page", 0).queryParam("size", 10)
        .when()
            .get(BASE + "/inactive")
    ...
    @Test
    void importProducts_validCsv_returns200WithImportCounts() {
        String csv = "sku,name,description,category,price,stock,active\n"
                + "SKU-CSV-1,CSV Product One,Desc,Tech,10.50,5,true\n"
                + "SKU-CSV-2,CSV Product Two,Desc,Tech,20.00,3,true\n"
                + ",Missing Sku,Desc,Tech,10.00,1,true\n";

        given()
            .header("Authorization", "Bearer " + TestTokenUtil.adminToken())
            .multiPart("file", "products.csv", csv.getBytes(java.nio.charset.StandardCharsets.UTF_8), "text/csv")
        .when()
            .post(BASE + "/import")
        .then()
            .statusCode(200)
            ...
            .body("data.errors", hasSize(1));
    }

    @Test
    void importProducts_missingFile_returns400() {
        given()
            .header("Authorization", "Bearer " + TestTokenUtil.adminToken())
        .when()
            .post(BASE + "/import")
    ...
    @Test
    void importProducts_noToken_returns401() {
        String csv = "sku,name,description,category,price,stock,active\n";
    ...
    @Test
    void importProducts_userRole_returns403() {
        String csv = "sku,name,description,category,price,stock,active\n";
    ...
    @Test
    void reportPdf_returns200WithPdfContentType() {
        createAndGetId();
        ...
        .then()
            ...
            .header("Content-Disposition", containsString("products-report.pdf"))
            .extract().asByteArray();

        assertThat(new String(pdf, 0, 5, java.nio.charset.StandardCharsets.US_ASCII), equalTo("%PDF-"));
    }

    @Test
    void reportPdf_noToken_returns401() {
        given()
        .when()
            .get(BASE + "/report/pdf")
        .then()
    ...
    @Test
    void reportExcel_returns200WithExcelContentType() {
        createAndGetId();

        given()
            .header("Authorization", "Bearer " + TestTokenUtil.userToken())
        .when()
            .get(BASE + "/report/excel")
        .then()
            .statusCode(200)
            .contentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            .header("Content-Disposition", containsString("products-report.xlsx"));
    }

    @Test
    void reportExcel_noToken_returns401() {
        given()
        .when()
            .get(BASE + "/report/excel")
        .then()
