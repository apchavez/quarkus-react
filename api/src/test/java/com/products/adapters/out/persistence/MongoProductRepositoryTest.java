...
class MongoProductRepositoryTest extends BaseMongoIntegrationTest {
    ...
    @Test
    void testFindAllActiveProducts_Success() {
        repository.insert(buildProduct("SKU-PAGE-001", "Paged Test"));

        var paged = repository.findAllActiveProducts(0, 10);

        assertThat(paged).isNotNull();
        assertThat(paged.data()).isNotEmpty();
        assertThat(paged.totalItems()).isGreaterThan(0);
        assertThat(paged.data()).allMatch(p -> p.active);
    }

    @Test
    void testFindAllInactiveProducts_Success() {
        Product inactive = buildProduct("SKU-INACTIVE-001", "Inactive Test");
        inactive.active = false;
        repository.insert(inactive);
        repository.insert(buildProduct("SKU-ACTIVE-001", "Active Test"));

        var paged = repository.findAllInactiveProducts(0, 10);

        assertThat(paged).isNotNull();
        assertThat(paged.data()).isNotEmpty();
        assertThat(paged.data()).allMatch(p -> !p.active);
        assertThat(paged.data()).extracting(p -> p.sku).contains("SKU-INACTIVE-001");
    }
    ...
    @Test
    void testFindAllActiveProducts_ServesStaleDataFromCache_UntilInvalidated() {
        Product product = buildProduct("SKU-CACHE-ACTIVE-001", "Original Active Name");
        repository.insert(product);

        // Populates the Redis cache entry for this page/size combo.
        var firstRead = repository.findAllActiveProducts(0, 10);
        assertThat(firstRead.data()).extracting(p -> p.name).contains("Original Active Name");

        // Mutate MongoDB directly, bypassing the repository (and its cache invalidation).
        mongoClient.getDatabase("products_test").getCollection("products")
                .updateOne(Filters.eq("sku", "SKU-CACHE-ACTIVE-001"), Updates.set("name", "Mutated Directly In Mongo"));

        // The cached (now stale) value is still served — proves Redis is actually being read.
        var staleRead = repository.findAllActiveProducts(0, 10);
        assertThat(staleRead.data()).extracting(p -> p.name).contains("Original Active Name");
        ...
        // ...so the next read reflects the fresh value from MongoDB.
        var freshRead = repository.findAllActiveProducts(0, 10);
        assertThat(freshRead.data()).extracting(p -> p.name).contains("Updated Via Repository");
    }
