...
import com.products.application.report.ExcelReportGenerator;
import com.products.application.report.PdfReportGenerator;
...
public class ProductResource {
    ...
    public Response insert(@Valid ProductRequest request) {
        LOG.infof("event=product_create_requested sku=%s", request.sku());
        try {
            ProductResponse created = productService.insert(request);
            LOG.infof("event=product_created id=%s sku=%s", created.id(), request.sku());
            return ApiResponse.created(Map.of("id", created.id()));
        } catch (RuntimeException e) {
            LOG.errorf(e, "event=product_create_failed sku=%s", request.sku());
            throw e;
        }
    }
    ...
    public Response update(
        ...
        LOG.infof("event=product_update_requested id=%s", id);
        try {
            productService.update(id, request);
            LOG.infof("event=product_updated id=%s", id);
            return ApiResponse.updated();
        } catch (RuntimeException e) {
            LOG.errorf(e, "event=product_update_failed id=%s", id);
            throw e;
        }
    }
    ...
    public Response delete(
            @Parameter(description = "24-character hexadecimal MongoDB ObjectId")
            @PathParam("id") @Pattern(regexp = "^[a-fA-F0-9]{24}$", message = "id has invalid format") String id) {
        LOG.infof("event=product_delete_requested id=%s", id);
        try {
            productService.delete(id);
            LOG.infof("event=product_deleted id=%s", id);
            return ApiResponse.deleted();
        } catch (RuntimeException e) {
            LOG.errorf(e, "event=product_delete_failed id=%s", id);
            throw e;
        }
    }
    ...
    public Response findAllActive(
        ...
        ProductsPagedResponse response = productService.findAllActive(page, size);
    ...
    @Path("/inactive")
    @RolesAllowed({"ADMIN", "USER"})
    @Operation(summary = "List inactive products (paginated)", description = "Admin-facing view for deactivated products — not cached.")
    @APIResponses({
        @APIResponse(responseCode = "200", description = "Paginated list of inactive products"),
        @APIResponse(responseCode = "400", description = "Invalid pagination parameters"),
        @APIResponse(responseCode = "401", description = "Missing or invalid JWT")
    })
    public Response findAllInactive(
            ...
            @Parameter(description = "Number of items per page — min 1, max 100 (default 10)")
            @QueryParam("size") @DefaultValue("10") @Min(value = 1, message = "size must be greater than 0") @Max(value = 100, message = "size must be at most 100") Integer size) {
        ProductsPagedResponse response = productService.findAllInactive(page, size);
        return ApiResponse.ok(response);
    }
    ...
    @Operation(summary = "Bulk import products from CSV",
            description = "Accepts a multipart file upload (field 'file') with header row "
                    + "sku,name,description,category,price,stock,active. Invalid rows are skipped and reported, "
                    + "not aborting the whole batch.")
    @APIResponses({
        @APIResponse(responseCode = "200", description = "Import finished — body reports imported/failed counts and per-row errors"),
        @APIResponse(responseCode = "400", description = "Missing file or unreadable CSV"),
        @APIResponse(responseCode = "401", description = "Missing or invalid JWT"),
        @APIResponse(responseCode = "403", description = "Insufficient role — ADMIN required")
    })
    public Response importProducts(@RestForm("file") FileUpload file) {
        if (file == null || file.uploadedFile() == null) {
            return ApiResponse.badRequest("file is required");
        }
        try (InputStream input = Files.newInputStream(file.uploadedFile())) {
            ImportResult result = productService.importProducts(input);
            LOG.infof("event=product_import_completed imported=%d failed=%d",
                    result.imported(), result.failed());
            return Response.ok(new ApiResponse(200, "Import completed", result)).build();
        } catch (IOException e) {
            LOG.errorf(e, "event=product_import_failed reason=unreadable_file");
            return ApiResponse.badRequest("could not read uploaded file");
        }
    }
    ...
    @RolesAllowed({"ADMIN", "USER"})
    @Operation(summary = "Download a PDF report of the product catalogue",
            description = "Streams a PDF listing every product with a total-inventory-value summary.")
    @APIResponses({
        @APIResponse(responseCode = "200", description = "PDF report generated"),
        @APIResponse(responseCode = "401", description = "Missing or invalid JWT")
    })
    public Response reportPdf() {
        List<ProductResponse> products = productService.findAllForReport();
        byte[] pdf = pdfReportGenerator.generate(products);
        return Response.ok(pdf)
                .header("Content-Disposition", "attachment; filename=\"products-report.pdf\"")
                .build();
    }
    ...
    @RolesAllowed({"ADMIN", "USER"})
    @Operation(summary = "Download an Excel report of the product catalogue",
            description = "Streams an XLSX listing every product with a total-inventory-value summary.")
    @APIResponses({
        @APIResponse(responseCode = "200", description = "Excel report generated"),
        @APIResponse(responseCode = "401", description = "Missing or invalid JWT")
    })
    public Response reportExcel() {
        List<ProductResponse> products = productService.findAllForReport();
        byte[] excel = excelReportGenerator.generate(products);
