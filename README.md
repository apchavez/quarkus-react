...
|---|---|
| Lenguaje / Runtime | Java 21, Quarkus 3 |
| Base de datos | MongoDB |
| Caché / Rate limiting | Redis — rate limiting + cache-aside para lecturas de productos |
| Mensajería | Apache Kafka (SmallRye Reactive Messaging, tópico `product-events`) |
| Seguridad | SmallRye JWT (RS256), CORS, rate limiting |
| Observabilidad | SmallRye Health, Micrometer + Prometheus, OpenTelemetry |
| Documentación de API | SmallRye OpenAPI (Swagger UI) |
| Reportes | Apache PDFBox / Apache POI |
| Procesamiento batch | Apache Commons CSV (bulk import) |
| Utilidades | MapStruct, Lombok |
| Build | Gradle, JaCoCo (incl. extensión `quarkus-jacoco` para cobertura del classloader aumentado) |
| Pruebas de integración | Testcontainers (MongoDB) |
...
| Categoría | Tecnología |
|---|---|
| Contenedores / Orquestación | Docker, Kubernetes (Helm) |
...
Asume que ya existe un clúster con `ingress-nginx`, `cert-manager`, y un `StorageClass` por defecto. Para levantar uno local sin costo:

```bash
kind create cluster --name product-management
...
|---|---|---|
| `ci.yml` | Cada push / PR a `main` | Tests del backend + gate de cobertura JaCoCo; typecheck, tests y cobertura del frontend; E2E con Playwright; SonarCloud (en main); (solo push a `main`) construye y publica `ghcr.io/apchavez/product-api:latest`/`:sha-<SHA>` y `ghcr.io/apchavez/product-web:latest`/`:sha-<SHA>` |
| `deploy.yml` | Manual (`workflow_dispatch`) | Levanta un clúster `kind` efímero en el propio runner, hace `helm template` + `kubectl apply` del chart, espera el rollout de `mongo`/`kafka`/`product-api`/`product-web`, corre un smoke test HTTP contra `/api/v1/q/health/ready`, y destruye el clúster al final |
| `k6.yml` | Manual (`workflow_dispatch`) | Corre `k6/product-api.js` contra `K6_BASE_URL` con el escenario elegido (`smoke`/`load`/`spike`) |

### Flujo de deploy

`deploy.yml` no depende de ningún clúster ni secreto externo — crea y destruye un `kind` efímero dentro del mismo job de CI, como validación end-to-end de que el Helm chart despliega y arranca correctamente. No hay clúster persistente detrás de este proyecto de portafolio.

```bash
gh workflow run deploy.yml
```

Para levantar el mismo chart en un `kind` local en tu laptop (fuera de CI):

```bash
kind create cluster --name product-management
helm upgrade --install product-management ./chart --namespace product-management --create-namespace \
  --set secrets.mongoUsername=<user> --set secrets.mongoPassword=<pass> \
  --set secrets.mongoConnectionString=<conn-string> \
  --set secrets.kafkaUser=<user> --set secrets.kafkaPassword=<pass>
kind delete cluster --name product-management   # para limpiar cuando termines
```
...
| Archivo | Descripción |
|---|---|
| `quarkus-react.postman_collection.json` | Colección principal (16 requests) |
...
- Bulk import de productos vía CSV (`POST /api/v1/products/import`) con validación fila por fila que no aborta el lote completo, y descarga de reportes del catálogo en PDF (Apache PDFBox) y Excel (Apache POI, streaming vía `SXSSFWorkbook`)
...
- Despliegue en Kubernetes vía Helm sobre cualquier clúster (local con `kind` o real), con `ingress-nginx` y `cert-manager` como dependencias externas del chart
...
Este repo hace pareja con **spring-webflux-angular**, **spring-mvc-angular**, **net-vue**, y **spring-jpa-native**: los cinco forman la familia de proyectos de Gestión de Productos — los primeros cuatro implementan el mismo dominio (sku/name/description/category/price/stock/active), los mismos 11 endpoints REST (crear, actualizar, eliminar, buscar por ID, buscar por SKU, buscar por prefijo de nombre, listar activos — paginado y cacheado, listar inactivos — paginado, bulk import vía CSV, descargar reporte PDF, descargar reporte Excel), el mismo tópico Kafka `product-events` y las mismas reglas de rate limiting con Redis, distinto stack de backend/frontend — mantenidos en paridad funcional a propósito; spring-jpa-native comparte el dominio de gestión de productos con un enfoque distinto, centrado en JPA/Hibernate (relaciones de entidades, fix de N+1, bloqueo optimista, auditoría) y un frontend móvil en React Native en vez de web. El trío serverless de agendamiento de citas forma un segundo grupo así, compartiendo ese dominio en su lugar.
