...
test.describe('Products', () => {
  ...
  test('toggling "Mostrar inactivos" switches between the active and inactive lists', async ({ page }) => {
    const inactiveProduct: Product = { ...PRODUCT, id: 2, sku: 'PROD-002', name: 'Widget Gamma', active: false };

    await page.route('**/api/v1/products/active**', async (route) => {
      await route.fulfill({ json: pageResponse([PRODUCT]) });
    });
    await page.route('**/api/v1/products/inactive**', async (route) => {
      await route.fulfill({ json: pageResponse([inactiveProduct]) });
    });

    await page.goto('/');

    await expect(page.getByText('Widget Alpha')).toBeVisible();
    await expect(page.getByText('Widget Gamma')).not.toBeVisible();

    await page.getByRole('checkbox', { name: 'Mostrar inactivos' }).check();

    await expect(page.getByText('Widget Gamma')).toBeVisible();
    await expect(page.getByText('Widget Alpha')).not.toBeVisible();
  });
});
