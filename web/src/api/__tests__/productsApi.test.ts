...
describe('getProducts', () => {
  it('returns mapped products on success', async () => {
    mockFetch.mockResolvedValue({
      ok: true,
      json: async () => ({
        data: { content: [mockProduct], page: 0, size: 10, totalElements: 25, totalPages: 3, last: false },
      }),
    });
  ...
  it('uses default page and size, and hits /active by default', async () => {
    mockFetch.mockResolvedValue({
      ok: true,
      json: async () => ({ data: { content: [], page: 0, size: 10, totalElements: 0, totalPages: 1, last: true } }),
    });

    await getProducts();

    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('/api/v1/products/active?page=0'),
      expect.anything()
    );
  });

  it('hits /inactive when showInactive is true', async () => {
    mockFetch.mockResolvedValue({
      ok: true,
      json: async () => ({ data: { content: [], page: 0, size: 10, totalElements: 0, totalPages: 1, last: true } }),
    });

    await getProducts(0, 10, true);

    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('/api/v1/products/inactive?page=0'),
      expect.anything()
    );
  });
