...
function renderTable(items: Product[] = products, showInactive = false) {
  return render(
    <ProductsTable
      products={items}
      onEdit={onEdit}
...
describe('ProductsTable — toggle mostrar inactivos', () => {
  it('muestra el checkbox desmarcado por defecto y llama onToggleShowInactive al hacer clic', async () => {
    renderTable();
    const checkbox = screen.getByRole('checkbox', { name: 'Mostrar inactivos' });
    expect(checkbox).not.toBeChecked();

    await userEvent.click(checkbox);
    expect(onToggleShowInactive).toHaveBeenCalledTimes(1);
  });

  it('muestra el checkbox marcado cuando showInactive es true', () => {
