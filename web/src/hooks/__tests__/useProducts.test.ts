...
describe('useProducts — carga inicial', () => {
  it('carga los productos al montar y actualiza totalPages', async () => {
    ...
    expect(api.getProducts).toHaveBeenCalledWith(0, 10, false);
...
describe('useProducts — paginación y estado de edición', () => {
  it('cambia de página y llama getProducts con el offset correcto', async () => {
    ...
    await waitFor(() =>
      expect(api.getProducts).toHaveBeenCalledWith(1, 10, false)
    );
  });

  it('toggleShowInactive alterna showInactive, resetea la página a 1 y llama getProducts con inactive=true', async () => {
    const { result } = renderHook(() => useProducts());
    await waitFor(() => expect(result.current.products).toHaveLength(1));
    ...
    await waitFor(() => expect(api.getProducts).toHaveBeenCalledWith(1, 10, false));

    act(() => {
      result.current.toggleShowInactive();
    });

    expect(result.current.showInactive).toBe(true);
    expect(result.current.page).toBe(1);
    await waitFor(() => expect(api.getProducts).toHaveBeenCalledWith(0, 10, true));
  });
